-- Corrige a RPC administrativa da rifa em ambientes Supabase onde pgcrypto
-- está instalado no schema `extensions` e a função usa search_path vazio.
--
-- Erro observado em produção:
--   function digest(text, unknown) does not exist
--
-- O schema é qualificado explicitamente para manter o search_path restrito.
...
create or replace function public.admin_create_dante_raffle_entry(
  ...
  p_reservation_mode text default 'without_expiration',
...
begin
  ...
  v_token_hash := encode(extensions.digest('admin_manual_' || v_reservation_id::text, 'sha256'), 'hex');
